...
// 1. Read and clean mixed encoding Markdown
const rawMdBuffer = fs.readFileSync(mdPath);
let cleanMdStr = '';
for (let i = 0; i < rawMdBuffer.length; i++) {
    if (rawMdBuffer[i] !== 0x00) {
        cleanMdStr += String.fromCharCode(rawMdBuffer[i]);
    }
}
// Fix any weird line endings
cleanMdStr = cleanMdStr.replace(/\r\n/g, '\n').replace(/\r/g, '\n');

// Append Database Schema missing from earlier translation pass
const schemaPTBR = `
---
...
try {
    const fixedPath = path.join(__dirname, 'Full_Course_Curriculum_PTBR_Fixed.md');
    fs.writeFileSync(fixedPath, cleanMdStr, 'utf8');
} catch (e) {
    console.log("Could not write fixed md");
}

// 2. Parse and generate JSONs for all 10 modules
...
function getLessonIdFromTitle(title) {
    // Basic slugification rules to match original backend lesson IDs
    let slug = title.toLowerCase()
        .replace(/[^a-z0-9\s-]/g, '')
        .trim()
        .replace(/\s+/g, '_');
    return `lesson_${slug.substring(0, 30)}`; // truncate
}

// Because the original English JSON has fixed IDs, let's just make new clean IDs for the PTBR ones 
// dynamically, or try to keep them consistent if possible. For simplicity, we just generate robust IDs.
const EN_LESSON_IDS = {
    // English mapping if we wanted to be strict, but generating fresh IDs is fine since UI uses courses.json
};
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (modMatch) {
        ...
        let title = modMatch[2] ? modMatch[2].trim() : modMatch[1].trim();
        let namePrefix = line.includes("Módulo") ? `Módulo ${modMatch[1]}: ` : `Ponto de Verificação: `
    ...
    if (lessonMatch && currentModuleObj) {
        ...
        currentLessonId = `lesson_${lessonCounter}_${lessonTitle.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 20)}`;
        lessonCounter++;
            ...
            exercises: [
                {
                    id: "challenge_1",
                    title: "Desafio 1",
                    prompt_markdown: "Escreva uma query para resolver o cenário acima.",
    ...
    if (currentLessonData && parsingLessonContent) {
        if (line.match(/^\*\*Desafios\*\*:/)) {
            // we skip inserting challenges into markdown explicitly and let the UI render them via JSON exercises
            continue;
        }
        if (line.startsWith('```sql')) {
            // If it's the `editor vazio` blocks, skip them in the content body 
            // because they are in the exercises array!
        }
...
function saveCurrentLesson() {
    if (currentLessonData && currentModuleId && currentLessonId) {
        // Strip hanging empty sql blocks
...
// Write the reconstructed courses.json
fs.writeFileSync(coursesJsonPath, JSON.stringify(coursesData, null, 2));
console.log('Successfully rebuilt Full_Course_Curriculum_PTBR_Fixed.md and ALL JSON files.');
